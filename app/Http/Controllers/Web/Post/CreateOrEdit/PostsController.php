...
class PostsController extends FrontController
{
	...
	public function __construct()
	{
		...
		$categoriesTypes = Cache::remember($cacheId, $this->cacheExpiration, function () {
			return DB::table('categories')->orderBy('lft')->where(function ($q) {
				$q->where('created_by', NULL)
					->orWhere('type', auth()->user()->id);
			})->get();
		});
		...
		$SkillsTypes = Cache::remember($cacheId, $this->cacheExpiration, function () {
			return DB::table('job_skills')->orderBy('id')->where(function ($q) {
				$q->where('created_by', NULL)
					->orWhere('type', auth()->user()->id);
			})->get();
		});
	...
	function updateChache()
	{
		...
		Cache::pull($cacheId);
		$categoriesTypes = Cache::remember($cacheId, $this->cacheExpiration, function () {
			return DB::table('categories')->orderBy('lft')->where('created_by', NULL)->orWhere('created_by', auth()->user()->id)->get();
		});
		view()->share('categoriesTypes', $categoriesTypes);
		...
		Cache::pull($cacheId);
		$SkillsTypes = Cache::remember($cacheId, $this->cacheExpiration, function () {
			return DB::table('job_skills')->orderBy('id')->where('created_by', NULL)->orWhere('created_by', auth()->user()->id)->get();
		});
		view()->share('SkillsTypes', $SkillsTypes);
	}
	/**
	...
	public function list()
	{
		...
		// dd($allPosts);
		return appView('post.List.list', compact('allPosts'));
	}
	...
	public function viewSinglePost(Request $request)
	{
		...
		$industeries = DB::table('categories')->where('created_by', NULL)->orWhere('created_by', auth()->user()->id)->get();
